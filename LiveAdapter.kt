...
class LiveAdapter private constructor(
    ...
    fun intoViewPager2(pager2: ViewPager2) = apply { pager2.adapter = this }
    ...
    override fun onBindViewHolder(holder: Holder<ViewDataBinding>, position: Int) {
        ...
        type?.let { getVariable(it) }?.let {
            holder.binding.setVariable(it, data?.value?.get(position)
                ?: list?.get(position))
        }
    ...
    override fun getItemViewType(position: Int) =
            layoutHandler?.getItemLayout((data?.value?.get(position) ?: (list?.get(position)
                    ?: Any())), position)
    ...
    private fun getType(position: Int) =
            typeHandler?.getItemType((data?.value?.get(position) ?: (list?.get(position)
                    ?: Any())), position)
                    ?: map[((data?.value?.get(position) ?: (list?.get(position)
                            ?: Any()))).javaClass]
